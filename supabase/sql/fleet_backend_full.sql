-- Fleet allocation backend bootstrap
-- Apply this file in Supabase SQL Editor or as a single migration.
...
alter table public.users
  drop constraint if exists users_linked_driver_id_fkey;

alter table public.users
  add constraint users_linked_driver_id_fkey
  foreign key (linked_driver_id) references public.drivers(id) on delete set null;
...
alter table public.drivers
  drop constraint if exists drivers_current_state_days_check;

alter table public.drivers
  add constraint drivers_current_state_days_check check (current_state_days >= 0);
...
create index if not exists drivers_state_started_on_idx on public.drivers (state_started_on);
create index if not exists drivers_roster_status_idx on public.drivers (roster_status);
...
create index if not exists vehiclesc_priority_idx on public.vehiclesc (vehicle_priority);
create index if not exists vehiclesc_status_idx on public.vehiclesc (status);

drop trigger if exists trg_vehiclesc_updated_at on public.vehiclesc;
create trigger trg_vehiclesc_updated_at
before update on public.vehiclesc
for each row execute function public.set_updated_at();
...
create unique index if not exists allocations_open_driver_uidx
on public.allocations (driver_id)
where ended_at is null and status in ('pending', 'active', 'locked');

create unique index if not exists allocations_open_vehicle_uidx
on public.allocations (vehicle_id)
where ended_at is null and status in ('pending', 'active', 'locked');

create index if not exists allocations_driver_status_idx on public.allocations (driver_id, status);
create index if not exists allocations_vehicle_status_idx on public.allocations (vehicle_id, status);
create index if not exists allocations_started_at_idx on public.allocations (started_at desc);
...
create index if not exists allocation_history_event_idx on public.allocation_history (event_type, created_at desc);
...
create unique index if not exists driver_state_history_open_uidx
on public.driver_state_history (driver_id)
where effective_to is null;

create index if not exists driver_state_history_driver_idx
on public.driver_state_history (driver_id, effective_from desc);
...
alter table public.driver_leave
  drop constraint if exists driver_leave_no_overlap;

alter table public.driver_leave
  add constraint driver_leave_no_overlap
  exclude using gist (
    driver_id with =,
    daterange(leave_start, leave_end, '[]') with &&
  )
  where (status in ('scheduled', 'active'));

create index if not exists driver_leave_driver_status_idx on public.driver_leave (driver_id, status);
create index if not exists driver_leave_window_idx on public.driver_leave (leave_start, leave_end) where status in ('scheduled', 'active');

drop trigger if exists trg_driver_leave_updated_at on public.driver_leave;
create trigger trg_driver_leave_updated_at
before update on public.driver_leave
for each row execute function public.set_updated_at();
...
create index if not exists employee_roster_import_rows_import_idx on public.employee_roster_import_rows (import_id, row_number);
create index if not exists employee_roster_import_rows_match_idx on public.employee_roster_import_rows (match_status, matched_driver_id);
...
create index if not exists driver_roster_periods_active_idx on public.driver_roster_periods (period_start, period_end);
...
create unique index if not exists cron_job_runs_one_running_uidx
...
alter table public.drivers
  drop constraint if exists drivers_current_leave_fk;
alter table public.drivers
  add constraint drivers_current_leave_fk
  foreign key (current_leave_id) references public.driver_leave(id) on delete set null;

alter table public.drivers
  drop constraint if exists drivers_current_allocation_fk;
alter table public.drivers
  add constraint drivers_current_allocation_fk
  foreign key (current_allocation_id) references public.allocations(id) on delete set null;

alter table public.drivers
  drop constraint if exists drivers_current_vehicle_fk;
alter table public.drivers
  add constraint drivers_current_vehicle_fk
  foreign key (current_vehicle_id) references public.vehiclesc(id) on delete set null;

alter table public.vehiclesc
  drop constraint if exists vehiclesc_current_allocation_fk;
alter table public.vehiclesc
  add constraint vehiclesc_current_allocation_fk
  foreign key (current_allocation_id) references public.allocations(id) on delete set null;

alter table public.vehiclesc
  drop constraint if exists vehiclesc_current_driver_fk;
alter table public.vehiclesc
  add constraint vehiclesc_current_driver_fk
...
create or replace function public.refresh_driver_snapshot(
  p_driver_id uuid,
...
begin
  ...
  if v_driver.current_leave_id is not null then
    raise exception 'Driver is on leave';
  end if;

  if v_driver.is_allocatable is false then
    raise exception 'Driver is not allocatable';
  end if;
  ...
  if exists (
    ...
    raise exception 'Driver already has an active allocation';
  end if;
  ...
  if exists (
    ...
    raise exception 'Vehicle already has an active allocation';
  end if;
  ...
  values (
    ...
    p_vehicle_id,
    p_actor_user_id,
    jsonb_build_object('notes', p_notes, 'allocation_type', p_allocation_type)
  );
  ...
  if (select current_state from public.drivers where id = p_driver_id) <> 'active' then
    perform public.upsert_driver_state_tx(
      p_driver_id,
      'active',
      'allocation_assigned',
      p_notes,
  ...
  select jsonb_build_object(
    'allocation', to_jsonb(a.*),
    'driver', (select to_jsonb(d.*) from public.drivers d where d.id = a.driver_id),
    'vehicle', (select to_jsonb(v.*) from public.vehiclesc v where v.id = a.vehicle_id)
  )
  into v_after
...
$$;
  ...
  if v_open_state.id is null then
    ...
    values (
      p_driver_id,
      coalesce(v_driver.current_state, 'off'),
      coalesce(v_driver.state_started_on, current_date),
      'system',
      'snapshot_seed',
      p_actor_user_id
    )
  ...
  select * into v_open_leave
  from public.driver_leave
  where driver_id = p_driver_id
    and status = 'active'
    and leave_start <= current_date
    and leave_end >= current_date
  order by leave_start desc
  limit 1;

  select * into v_open_allocation
  ...
  select drp.period_end into v_roster_end
  from public.driver_roster_periods drp
  where drp.driver_id = p_driver_id
    and drp.period_start <= current_date
    and drp.period_end >= current_date
  order by drp.period_end desc
  limit 1;
  ...
  v_available := v_open_state.state in ('active', 'off') and v_open_leave.id is null;
  v_allocatable := v_open_state.state in ('active', 'off')
    and v_open_leave.id is null
    and v_roster_end is not null
    and v_open_allocation.id is null;
...
create or replace function public.reassign_driver_tx(
  p_vehicle_id uuid,
  p_new_driver_id uuid,
  p_effective_from date default current_date,
  p_notes text default null,
  p_actor_user_id uuid default null,
  p_allow_override_locked boolean default false
)
returns jsonb
language plpgsql
as $$
...
begin
  ...
  if found then
    ...
    values (
      v_current_allocation.id,
      'allocation_reassigned_out',
      v_current_allocation.driver_id,
      v_current_allocation.vehicle_id,
      p_actor_user_id,
      jsonb_build_object('notes', p_notes, 'new_driver_id', p_new_driver_id)
    );
...
create or replace function public.mark_driver_leave_tx(
  p_driver_id uuid,
  p_leave_start date,
  p_leave_end date,
  p_reason text default null,
  p_notes text default null,
  p_force_end_allocation boolean default false,
  p_actor_user_id uuid default null
)
returns jsonb
language plpgsql
...
begin
  ...
  insert into public.driver_leave (
    driver_id,
    leave_start,
    leave_end,
    status,
  ...
  values (
    ...
    p_leave_start,
    p_leave_end,
    case when p_leave_start <= current_date and p_leave_end >= current_date then 'active' else 'scheduled' end,
    p_reason,
    p_notes,
    p_actor_user_id,
    p_actor_user_id
  ...
  perform public.write_audit_log(
    ...
    p_actor_user_id,
    null,
    (select to_jsonb(dl.*) from public.driver_leave dl where dl.id = v_leave_id),
    jsonb_build_object('driver_id', p_driver_id, 'reason', p_reason, 'notes', p_notes)
  );

  return jsonb_build_object(
    'leave_id', v_leave_id,
    'driver', (select to_jsonb(d.*) from public.drivers d where d.id = p_driver_id),
    'leave', (select to_jsonb(dl.*) from public.driver_leave dl where dl.id = v_leave_id)
  );
end;
$$;

create or replace function public.end_driver_leave_tx(
  p_leave_id uuid,
...
begin
  ...
  if not found then
    raise exception 'Leave not found: %', p_leave_id;
  end if;

  update public.driver_leave
  set
    leave_end = least(v_leave.leave_end, coalesce(p_end_date, current_date)),
    status = 'completed',
    notes = coalesce(p_notes, notes)
  where id = p_leave_id;
  ...
  if exists (
    ...
    where id = v_leave.driver_id
      and current_state = 'on_leave'
  ) then
    perform public.upsert_driver_state_tx(
      v_leave.driver_id,
      'off',
  ...
  perform public.write_audit_log(
    ...
    p_actor_user_id,
    null,
    (select to_jsonb(dl.*) from public.driver_leave dl where dl.id = p_leave_id),
    jsonb_build_object('notes', p_notes)
  );

  return jsonb_build_object(
    'leave', (select to_jsonb(dl.*) from public.driver_leave dl where dl.id = p_leave_id),
    'driver', (select to_jsonb(d.*) from public.drivers d where d.id = v_leave.driver_id)
  );
end;
$$;

create or replace function public.start_cron_job_run(
  p_job_name text,
...
declare
  v_id uuid;
begin
  insert into public.cron_job_runs (job_name, status, triggered_by)
  values (p_job_name, 'running', p_triggered_by)
...
create or replace function public.finish_cron_job_run(
  p_run_id uuid,
  p_status text,
...
select
  ...
  d.current_state,
  d.state_started_on,
  d.current_state_days,
  d.warning_level,
  d.available,
  d.is_allocatable,
  d.current_leave_id,
  d.current_vehicle_id,
  d.current_allocation_id,
  d.roster_status,
  d.current_roster_period_end,
  case
    when d.current_state = 'active' then greatest(0, 33 - d.current_state_days)
    when d.current_state = 'off' then greatest(0, 7 - d.current_state_days)
    else null
  end as days_left
...
create index if not exists vehicle_rotation_plan_items_due_idx
on public.vehicle_rotation_plan_items (status, proposed_change_date);

create index if not exists vehicle_rotation_plan_items_vehicle_idx
on public.vehicle_rotation_plan_items (vehicle_id, status);

create index if not exists vehicle_rotation_plan_items_next_driver_idx
on public.vehicle_rotation_plan_items (next_driver_id, status);

drop trigger if exists trg_vehicle_rotation_plan_items_updated_at on public.vehicle_rotation_plan_items;
create trigger trg_vehicle_rotation_plan_items_updated_at
before update on public.vehicle_rotation_plan_items
for each row execute function public.set_updated_at();

create or replace function public.refresh_vehicle_snapshot(
  p_vehicle_id uuid
)
returns void
language plpgsql
...
begin
  ...
  set
    ...
    driver_name = case
      when v_driver.id is null then null
      else concat_ws(' ', v_driver.first_name, v_driver.surname)
    end,
...
create or replace function public.upsert_driver_state_tx(
  p_driver_id uuid,
  p_new_state public.driver_state,
  p_reason_code text,
...
begin
  ...
  perform public.write_audit_log(
    ...
    v_before,
    v_after,
    jsonb_build_object('reason_code', p_reason_code, 'source', p_source, 'note', p_note)
  );
...
create or replace function public.remove_allocation_tx(
...
begin
  ...
  if v_allocation.ended_at is not null then
    return jsonb_build_object('allocation_id', v_allocation.id, 'changed', false);
  end if;
  ...
  values (
    p_allocation_id,
    'allocation_removed',
    v_allocation.driver_id,
    v_allocation.vehicle_id,
    p_actor_user_id,
    jsonb_build_object('ended_reason', p_ended_reason, 'notes', p_notes)
  );
  ...
  if exists (
    select 1 from public.drivers
    where id = v_allocation.driver_id
      and current_state not in ('on_leave', 'suspended')
  ) then
    perform public.upsert_driver_state_tx(
      v_allocation.driver_id,
      'off',
